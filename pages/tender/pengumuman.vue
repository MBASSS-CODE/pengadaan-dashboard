<template>
  <div class="p-6 max-w-7xl mx-auto">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
      <div>
        <h1 class="text-2xl font-bold text-[color:hsl(var(--maz-foreground))]">Pengumuman Tender</h1>
        <p class="text-sm text-[color:hsl(var(--maz-muted))] mt-1">Daftar pengumuman paket pengadaan tender</p>
      </div>
    ...
    <div class="bg-[color:hsl(var(--maz-background))] rounded-xl border border-[color:hsl(var(--maz-border))] shadow-sm overflow-hidden">
      ...
      <div class="p-4 border-b border-[color:hsl(var(--maz-border))] bg-[color:hsl(var(--maz-background))] flex flex-col lg:flex-row gap-4 items-end">
        <div class="w-full lg:w-1/4">
          ...
          <MazInput 
            v-model="searchQuery" 
            placeholder="Cari paket, satker, ID tender..." 
            size="sm"
      ...
      <div v-else class="overflow-x-auto w-full">
        <MazTable
          ...
          :headers="[
            ...
            { label: 'Satuan Kerja & Detail', key: 'satker', sortable: false, classes: 'min-w-[220px]' },
        ...
        >
          ...
          <template #cell-paket="{ row }">
            <div 
              ...
              @click="row.url_lpse ? openUrl(row.url_lpse) : null"
            >
              {{ row.nama_paket || '-' }}
            </div>
            <div class="flex items-center gap-1.5 mt-2 flex-wrap">
              ...
              <span v-if="row.kd_tender" class="px-2 py-0.5 rounded text-[10px] font-medium bg-[color:hsl(var(--maz-foreground)_/_5%)] text-[color:hsl(var(--maz-muted))] border border-[color:hsl(var(--maz-border))]">
            ...
            <div class="text-xs text-[color:hsl(var(--maz-muted))] mt-1">
              {{ row.jenis_pengadaan || row.kgr_nama || '-' }}
            </div>
            <div class="flex items-center gap-3 text-[11px] text-[color:hsl(var(--maz-muted))] mt-1">
              ...
              <span v-if="row.kontrak_pembayaran">Kontrak: <strong class="text-[color:hsl(var(--maz-foreground))]">{{ row.kontrak_pembayaran }}</strong></span>
            </div>
          </template>
          ...
          <template #cell-satker="{ row }">
            ...
            <div class="text-xs text-[color:hsl(var(--maz-muted))] mt-1">
              {{ row.mtd_pemilihan || '-' }}
              <span v-if="row.mtd_evaluasi" class="text-[11px] text-[color:hsl(var(--maz-muted))]">({{ row.mtd_evaluasi }})</span>
            </div>
          ...
          <template #cell-status="{ row }">
            <div class="flex flex-col items-center gap-1.5 w-full">
              <span 
                ...
                :class="{
                  'bg-[color:hsl(var(--maz-success)_/_15%)] text-[color:hsl(var(--maz-success)_/_100%)] dark:bg-[color:hsl(var(--maz-success)_/_20%)]': row.status_tender === 'Selesai',
                  'bg-[color:hsl(var(--maz-primary)_/_15%)] text-[color:hsl(var(--maz-primary)_/_100%)] dark:bg-[color:hsl(var(--maz-primary)_/_20%)]': row.status_tender === 'Berlangsung',
                  'bg-[color:hsl(var(--maz-muted)_/_15%)] text-[color:hsl(var(--maz-foreground)_/_80%)] dark:bg-[color:hsl(var(--maz-muted)_/_20%)]': !['Selesai', 'Berlangsung'].includes(row.status_tender)
                }"
              >
                {{ row.status_tender || 'Unknown' }}
              </span>
              <span v-if="row.tgl_pengumuman_tender" class="text-[10px] text-[color:hsl(var(--maz-muted))] text-center mt-1">
                {{ formatDate(row.tgl_pengumuman_tender) }}
              </span>
            </div>
          </template>
        </MazTable>
...
const loadData = async (force = false) => {
  ...
  error.value = false;
  try {
    const response = await $fetch('/api/data/tender/pengumuman', {
      params: { 
        tahun: selectedYear.value,
        page: currentPage.value,
        limit: itemsPerPage.value,
        search: searchQuery.value || undefined,
        filterStatusTender: selectedStatus.value !== 'ALL' ? selectedStatus.value : undefined,
    ...
    if (response.filterOptions) {
      const statusList = response.filterOptions.statusTender || response.filterOptions.statusNontender;
      if (statusList) {
        statusOptions.value = [
          { label: 'Semua Status', value: 'ALL' },
          ...statusList.map(opt => ({ label: opt, value: opt }))
        ];
      }
