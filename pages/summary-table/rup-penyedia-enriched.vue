  ...
  <div class="p-6 max-w-[1400px] mx-auto">
    ...
    <div class="mb-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
      ...
      <div class="flex items-center gap-2">
        <MazBtn @click="showFilters = !showFilters" :color="showFilters ? 'primary' : 'white'" size="sm" outline>
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" />
          </svg>
          Filter Lanjutan
        </MazBtn>
        <MazBtn v-if="hasActiveFilters" @click="resetFilters" color="danger" size="sm" outline>Reset</MazBtn>
      </div>
    </div>

    <!-- Advanced Filters Panel -->
    <div v-if="showFilters" class="mb-6 p-4 bg-[color:hsl(var(--maz-background))] rounded-xl border border-[color:hsl(var(--maz-border))] shadow-sm transition-all">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div>
          <label class="block text-[10px] font-bold text-[color:hsl(var(--maz-muted))] uppercase tracking-wider mb-1">Status Realisasi</label>
          <MazSelect v-model="filterStatusRealisasi" :options="statusRealisasiOptions" size="sm" clearable @update:model-value="onFilterChange" multiple />
        </div>
        <div>
          <label class="block text-[10px] font-bold text-[color:hsl(var(--maz-muted))] uppercase tracking-wider mb-1">Sumber Dana</label>
          <MazSelect v-model="filterSumberDana" :options="sumberDanaOptions" size="sm" clearable @update:model-value="onFilterChange" multiple />
        </div>
        <div>
          <label class="block text-[10px] font-bold text-[color:hsl(var(--maz-muted))] uppercase tracking-wider mb-1">Metode Pengadaan</label>
          <MazSelect v-model="filterMetodePengadaan" :options="metodePengadaanOptions" size="sm" clearable @update:model-value="onFilterChange" multiple />
        </div>
        <div>
          <label class="block text-[10px] font-bold text-[color:hsl(var(--maz-muted))] uppercase tracking-wider mb-1">Jenis Pengadaan</label>
          <MazSelect v-model="filterJenisPengadaan" :options="jenisPengadaanOptions" size="sm" clearable @update:model-value="onFilterChange" multiple />
        </div>
      </div>
    </div>
