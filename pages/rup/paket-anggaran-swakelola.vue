<template>
  <div class="p-6 max-w-7xl mx-auto">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
      <div>
        <h1 class="text-2xl font-bold text-[color:hsl(var(--maz-foreground))]">Paket Anggaran Swakelola</h1>
        <p class="text-sm text-[color:hsl(var(--maz-muted))] mt-1">Daftar pagu anggaran dan MAK untuk paket swakelola</p>
      </div>
    ...
    <div class="bg-[color:hsl(var(--maz-background))] rounded-xl border border-[color:hsl(var(--maz-border))] shadow-sm overflow-hidden">
      ...
      <div v-else class="overflow-x-auto w-full">
        ...
        :headers="[
          ...
          { label: 'Kode RUP', key: 'kd_rup', sortable: false },
          { label: 'Satuan Kerja', key: 'satker', sortable: false, classes: 'min-w-[200px]' },
          { label: 'MAK & Sumber Dana', key: 'mak', sortable: false },
      ...
      >
        ...
        <template #cell-kd_rup="{ row }">
          <span class="font-semibold text-[color:hsl(var(--maz-primary))]">{{ row.kd_rup }}</span>
        </template>
        
        <template #cell-satker="{ row }">
          <div class="font-medium truncate max-w-xs" :title="row.nama_satker">{{ row.nama_satker }}</div>
          <div class="text-xs text-[color:hsl(var(--maz-muted))] mt-1">Kode: {{ row.kd_satker_str }}</div>
        </template>
        
        <template #cell-mak="{ row }">
          <div class="font-mono text-xs p-1 bg-[color:hsl(var(--maz-foreground)_/_5%)] rounded truncate max-w-[200px]" :title="row.mak">
            {{ row.mak }}
          </div>
          <div class="text-xs text-[color:hsl(var(--maz-muted))] mt-1">
            {{ row.sumber_dana }} <span v-if="row.jenis_dana_apbn">({{ row.jenis_dana_apbn }})</span>
          </div>
        </template>
        
        <template #cell-pagu="{ row }">
          <span class="font-medium">{{ formatRupiah(row.pagu) }}</span>
        </template>
        ...
        <template #cell-status="{ row }">
          <div class="flex flex-col items-center gap-1">
            <span 
              ...
              :class="{
                'bg-[color:hsl(var(--maz-success)_/_15%)] text-[color:hsl(var(--maz-info)_/_100%)] dark:bg-[color:hsl(var(--maz-success)_/_20%)]': row.status_aktif_rup,
                'bg-[color:hsl(var(--maz-destructive)_/_15%)] text-[color:hsl(var(--maz-destructive)_/_100%)] dark:bg-[color:hsl(var(--maz-destructive)_/_20%)]': !row.status_aktif_rup
              }"
            >
              {{ row.status_aktif_rup ? 'Aktif' : 'Non-Aktif' }}
            </span>
            <span 
              class="px-2.5 py-1 text-[0.7rem] font-semibold rounded-full w-full text-center border border-transparent"
              :class="{
...
const loadData = async (force = false) => {
  ...
  error.value = false;
  try {
    const response = await $fetch('/api/data/rup/paket-anggaran-swakelola', {
      params: { 
