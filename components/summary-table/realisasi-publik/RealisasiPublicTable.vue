          ...
          <table class="w-full text-left border-collapse">
            ...
            <tbody class="align-top relative">
              ...
              <tr v-else-if="items.length === 0 && !loading" class="border-b border-[color:hsl(var(--maz-border))]">
                <td colspan="5" class="py-12 text-center text-[color:hsl(var(--maz-muted))]">
              ...
              <template v-for="(item, index) in items" :key="index + '-' + item.kode_paket">
                <tr class="border-b border-[color:hsl(var(--maz-border))] hover:bg-[color:hsl(var(--maz-foreground)_/_2%)] transition-colors group">
                  ...
                  <td class="py-4 px-4">
                    <div class="font-semibold text-[color:hsl(var(--maz-primary))] mb-2 truncate max-w-[250px]" :title="item.nama_penyedia || '-'">
                      {{ item.nama_penyedia || '-' }}
                    </div>
                    <div class="flex flex-wrap gap-1 mt-1 text-[10px]">
                      ...
                      <span class="px-2 py-0.5 rounded-full bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800">
            ...
            <div class="bg-[color:hsl(var(--maz-background))] border border-[color:hsl(var(--maz-border))] rounded-xl p-4 shadow-sm flex flex-col gap-4">
              ...
              <div class="grid grid-cols-2 gap-3 text-xs">
                ...
                <div class="flex flex-col gap-1">
                  ...
                  <span class="font-medium text-[color:hsl(var(--maz-primary))] truncate" :title="item.nama_penyedia || '-'">{{ item.nama_penyedia || '-' }}</span>
                </div>
                <div class="flex flex-col gap-1">
                ...
                <div class="flex flex-col gap-1">
                  <span class="text-[color:hsl(var(--maz-muted))]">Tahapan</span>
                  <span class="font-medium text-[color:hsl(var(--maz-foreground))]">{{ item.tahapan_pengadaan || '-' }}</span>
                </div>
              </div>
              ...
              <div class="bg-[color:hsl(var(--maz-foreground)_/_3%)] rounded-lg p-3 mt-2 border border-[color:hsl(var(--maz-border))] flex flex-col gap-2">
                <div class="flex justify-between items-center">
                  <span class="text-xs font-semibold text-[color:hsl(var(--maz-muted))] uppercase">Total Nilai</span>
                  <span class="text-base font-black text-[color:hsl(var(--maz-foreground))]">{{ formatRupiah(item.total_nilai) }}</span>
                </div>
                <div class="h-px w-full bg-[color:hsl(var(--maz-border))]"></div>
                <div class="flex justify-between items-center text-xs">
                  <span class="text-[color:hsl(var(--maz-muted))]">Nilai PDN:</span>
                  <span class="font-semibold" :class="item.nilai_pdn > 0 ? 'text-teal-600 dark:text-teal-400' : ''">{{ formatRupiah(item.nilai_pdn) }}</span>
                </div>
                <div class="flex justify-between items-center text-xs">
                  <span class="text-[color:hsl(var(--maz-muted))]">Nilai UMK:</span>
                  <span class="font-semibold" :class="item.nilai_umk > 0 ? 'text-purple-600 dark:text-purple-400' : ''">{{ formatRupiah(item.nilai_umk) }}</span>
                </div>
              </div>
            </div>
          </template>
    ...
    <MazDialog v-model="exportModal" title="Export ke Excel (XLSX)">
      <div class="flex flex-col gap-4 py-2">
        <p class="text-sm text-[color:hsl(var(--maz-muted))]">
          Pilih mode ekspor data Realisasi Pengadaan (Public) untuk Tahun Anggaran {{ selectedYear }}:
        </p>
...
const loadData = async () => {
  ...
  try {
    ...
    const res = await $fetch('/api/summary-table/realisasi-publik', { params });
    if (res.success) {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      const wscols = [
        ...
        {wch: 20}, {wch: 15}, {wch: 30}, {wch: 20}, {wch: 20}, {wch: 50},
        {wch: 15}, {wch: 20}, {wch: 20}, {wch: 20}, {wch: 20}
      ];
      ws['!cols'] = wscols;

      const filename = `Realisasi_Pengadaan_Public_${selectedYear.value}${exportMode.value === 'filtered' ? '_Filtered' : ''}.xlsx`;
