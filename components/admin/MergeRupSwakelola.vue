<template>
  <div class="bg-[color:hsl(var(--maz-background))] rounded-xl border border-[color:hsl(var(--maz-border))] shadow-sm overflow-hidden">
            <div class="p-6 border-b border-[color:hsl(var(--maz-border))] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h2 class="text-lg font-bold text-[color:hsl(var(--maz-foreground))] flex items-center">
                  ...
                  Integrasi Data Master: RUP Swakelola
                </h2>
                <p class="text-xs text-[color:hsl(var(--maz-muted))] mt-1">Gabungkan RUP Swakelola (sebagai data induk) dengan Satker, Anggaran, Kaji Ulang, PPK, Paket Penyedia dan Pencatatan Pelaksanaan</p>
              </div>
                ...
                <div class="flex items-center gap-2">
                  ...
                  <div class="flex bg-[color:hsl(var(--maz-foreground)_/_5%)] p-1 rounded-lg">
                    <button v-for="y in availableYears" :key="y"
                      @click="selectedYear = y; fetchRpMergeStatus()"
            ...
            <div class="p-6">
              <!-- Alur / Stepper -->
              <div class="mb-6">
                <h3 class="text-sm font-bold uppercase tracking-wider text-[color:hsl(var(--maz-muted))] mb-3">Alur Integrasi RUP Swakelola</h3>
...
const rpMergeSteps = computed(() => {
  const hasData = rpMergeStatus.value.prerequisites?.some(s => s.found && s.endpoint === 'paket-swakelola');
  const hasPpk = rpMergeStatus.value.prerequisites?.find(s => s.endpoint === 'ppk-master');
  const lastMerge = rpMergeStatus.value.lastMerge;

  return [
    { label: '1. Sync RUP Swakelola', status: hasData ? 'done' : 'waiting' },
...
const fetchRpMergeStatus = async () => {
  try {
    ...
    const res = await $fetch('/api/admin/merge-rup-swakelola-status', { params: { tahun } });
    if (res.success) {
      rpMergeStatus.value = res.data;
    }
  } catch (error) {
    console.error('Failed to load RP Swakelola merge status', error);
  }
};
...
const runRpMerge = async () => {
  ...
  try {
    const tahun = selectedYear.value;
    const res = await $fetch('/api/admin/merge-rup-swakelola', {
      method: 'POST',
      body: { tahun }
    });
