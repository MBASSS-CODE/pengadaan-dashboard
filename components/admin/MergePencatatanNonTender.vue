            ...
            <div class="p-6 border-b border-[color:hsl(var(--maz-border))] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h2 class="text-lg font-bold text-[color:hsl(var(--maz-foreground))] flex items-center">
                  ...
                  Integrasi Data: Pencatatan Non-Tender
                </h2>
            ...
            <div class="p-6">
              ...
              <div class="mb-6">
                ...
                <div class="w-full overflow-x-auto pb-2">
                  <div class="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-0 sm:w-max">
                    <div v-for="(step, idx) in pcnMergeSteps" :key="idx" class="flex items-center shrink-0">
                      ...
                      <svg v-if="idx < pcnMergeSteps.length - 1" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mx-1 text-[color:hsl(var(--maz-muted))] hidden sm:block shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div class="mb-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  <div v-for="src in pcnMergeStatus.prerequisites" :key="src.endpoint"
              ...
              <!-- Last Merge Result -->
              <div v-if="pcnMergeStatus.lastMerge" class="mb-6">
                ...
                <div class="p-4 rounded-lg border border-[color:hsl(var(--maz-border))] bg-[color:hsl(var(--maz-foreground)_/_2%)]">
                  <div class="flex flex-wrap gap-x-6 gap-y-2 text-sm">
                    <span><strong>Waktu:</strong> {{ formatDate(pcnMergeStatus.lastMerge.timestamp) }}</span>
                    <span><strong>Trigger:</strong> {{ pcnMergeStatus.lastMerge.trigger }}</span>
                    <span><strong>Durasi:</strong> {{ pcnMergeStatus.lastMerge.duration_ms }}ms</span>
                    <span><strong>Total:</strong> {{ pcnMergeStatus.lastMerge.result?.total_records }} records</span>
                    <span>
                      <strong>Status:</strong>
                      <span :class="pcnMergeStatus.lastMerge.status === 'success' ? 'text-green-600' : 'text-red-600'">{{ pcnMergeStatus.lastMerge.status }}</span>
                    </span>
                  </div>
                </div>
              </div>
              ...
              <!-- Merge History -->
              <div v-if="pcnMergeStatus.history?.length">
                ...
                <div class="overflow-x-auto">
                  <table class="w-full text-left text-sm text-[color:hsl(var(--maz-foreground))]">
                    ...
                    <tbody>
                      <tr v-for="h in pcnMergeStatus.history" :key="h.id" class="border-b border-[color:hsl(var(--maz-border))] hover:bg-[color:hsl(var(--maz-foreground)_/_3%)]">
              ...
              <div v-if="pcnMergeMessage" class="mt-4 p-3 rounded-lg text-sm" :class="pcnMergeSuccess ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'">
                {{ pcnMergeMessage }}
              </div>
            </div>
          </div>
...
const pcnMergeLoading = ref(false);
const pcnMergeMessage = ref('');
const pcnMergeSuccess = ref(false);
const pcnMergeStatus = ref({
  prerequisites: [],
...
const pcnMergeSteps = computed(() => {
  const hasData = pcnMergeStatus.value.prerequisites?.some(s => s.found && s.endpoint === 'pencatatan-non-tender');
  const hasPpk = pcnMergeStatus.value.prerequisites?.find(s => s.endpoint === 'ppk-master');
  const lastMerge = pcnMergeStatus.value.lastMerge;

  return [
    { label: '1. Sync Data', status: hasData ? 'done' : 'waiting' },
    { label: '2. Data Dukung', status: pcnMergeStatus.value.allRequiredFound ? 'done' : 'waiting' },
...
const fetchPcnMergeStatus = async () => {
  try {
    ...
    console.error('Failed to load PCN merge status', error);
  }
};

const runPcnMerge = async () => {
  ...
  try {
    ...
    pcnMergeSuccess.value = res.success;
    pcnMergeMessage.value = res.message;
    if (res.success) {
      await fetchPcnMergeStatus();
      setTimeout(() => { pcnMergeMessage.value = ''; }, 5000);
    }
  } catch (error) {
    pcnMergeSuccess.value = false;
    pcnMergeMessage.value = 'Terjadi kesalahan saat menjalankan merge';
  } finally {
    pcnMergeLoading.value = false;
  }
};
