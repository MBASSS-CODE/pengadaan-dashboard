<template>
  <div class="bg-[color:hsl(var(--maz-background))] rounded-xl border border-[color:hsl(var(--maz-border))] shadow-sm overflow-hidden">
            <div class="p-6 border-b border-[color:hsl(var(--maz-border))] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h2 class="text-lg font-bold text-[color:hsl(var(--maz-foreground))] flex items-center">
                  ...
                  Integrasi Data Master: E-Purchasing
                </h2>
                <p class="text-xs text-[color:hsl(var(--maz-muted))] mt-1">Integrasikan transaksi E-Katalog dengan profil Penyedia dan RUP Penyedia</p>
              </div>
              <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                <div class="flex items-center gap-2">
                  ...
                  <div class="flex bg-[color:hsl(var(--maz-foreground)_/_5%)] p-1 rounded-lg">
                    <button v-for="y in availableYears" :key="y"
                      @click="selectedYear = y; fetchepMergeStatus()"
                ...
                <MazBtn @click="runepMerge" :loading="epMergeLoading" color="secondary" size="sm" :disabled="!epMergeStatus.allRequiredFound">
            ...
            <div class="p-6">
              <!-- Alur / Stepper -->
              <div class="mb-6">
                <h3 class="text-sm font-bold uppercase tracking-wider text-[color:hsl(var(--maz-muted))] mb-3">Alur Integrasi E-Purchasing</h3>
                <div class="w-full overflow-x-auto pb-2">
                  <div class="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-0 sm:w-max">
                    <div v-for="(step, idx) in epMergeSteps" :key="idx" class="flex items-center shrink-0">
                      ...
                      <svg v-if="idx < epMergeSteps.length - 1" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mx-1 text-[color:hsl(var(--maz-muted))] hidden sm:block shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div class="mb-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  <div v-for="src in epMergeStatus.prerequisites" :key="src.endpoint"
              ...
              <!-- Last Merge Result -->
              <div v-if="epMergeStatus.lastMerge" class="mb-6">
                ...
                <div class="p-4 rounded-lg border border-[color:hsl(var(--maz-border))] bg-[color:hsl(var(--maz-foreground)_/_2%)]">
                  <div class="flex flex-wrap gap-x-6 gap-y-2 text-sm">
                    <span><strong>Waktu:</strong> {{ formatDate(epMergeStatus.lastMerge.timestamp) }}</span>
                    <span><strong>Trigger:</strong> {{ epMergeStatus.lastMerge.trigger }}</span>
                    <span><strong>Durasi:</strong> {{ epMergeStatus.lastMerge.duration_ms }}ms</span>
                    <span><strong>Total:</strong> {{ epMergeStatus.lastMerge.result?.total_records }} records</span>
                    <span>
                      <strong>Status:</strong>
                      <span :class="epMergeStatus.lastMerge.status === 'success' ? 'text-green-600' : 'text-red-600'">{{ epMergeStatus.lastMerge.status }}</span>
                    </span>
                  </div>
                </div>
              </div>
              ...
              <!-- Merge History -->
              <div v-if="epMergeStatus.history?.length">
                ...
                <div class="overflow-x-auto">
                  <table class="w-full text-left text-sm text-[color:hsl(var(--maz-foreground))]">
                    ...
                    <tbody>
                      <tr v-for="h in epMergeStatus.history" :key="h.id" class="border-b border-[color:hsl(var(--maz-border))] hover:bg-[color:hsl(var(--maz-foreground)_/_3%)]">
              ...
              <div v-if="epMergeMessage" class="mt-4 p-3 rounded-lg text-sm" :class="epMergeSuccess ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'">
                {{ epMergeMessage }}
              </div>
            </div>
          </div>
...
const epMergeLoading = ref(false);
const epMergeMessage = ref('');
const epMergeSuccess = ref(false);
const epMergeStatus = ref({
  prerequisites: [],
...
const epMergeSteps = computed(() => {
  const hasData = epMergeStatus.value.prerequisites?.some(s => s.found && s.endpoint === 'paket-e-purchasing');
  const hasPpk = epMergeStatus.value.prerequisites?.find(s => s.endpoint === 'penyedia-master');
  const lastMerge = epMergeStatus.value.lastMerge;

  return [
    { label: '1. Sync E-Katalog', status: hasData ? 'done' : 'waiting' },
    { label: '2. Data Dukung', status: epMergeStatus.value.allRequiredFound ? 'done' : 'waiting' },
    { label: '3. Penyedia Terverifikasi', status: hasPpk?.count > 0 ? 'done' : 'waiting' },
...
const fetchepMergeStatus = async () => {
  try {
    const tahun = selectedYear.value;
    const res = await $fetch('/api/admin/merge-epurchasing-status', { params: { tahun } });
    if (res.success) {
      epMergeStatus.value = res.data;
    }
  } catch (error) {
...
const runepMerge = async () => {
  epMergeLoading.value = true;
  epMergeMessage.value = '';
  try {
    const tahun = selectedYear.value;
    const res = await $fetch('/api/admin/merge-epurchasing', {
      method: 'POST',
      body: { tahun }
    });
    epMergeSuccess.value = res.success;
    epMergeMessage.value = res.message;
    if (res.success) {
      await fetchepMergeStatus();
      setTimeout(() => { epMergeMessage.value = ''; }, 5000);
    }
  } catch (error) {
    epMergeSuccess.value = false;
    epMergeMessage.value = 'Terjadi kesalahan saat menjalankan merge';
  } finally {
    epMergeLoading.value = false;
  }
};
