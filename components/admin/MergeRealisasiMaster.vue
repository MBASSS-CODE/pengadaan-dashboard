    ...
    <div class="p-6 border-b border-[color:hsl(var(--maz-border))] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <div>
        <h2 class="text-lg font-bold text-[color:hsl(var(--maz-foreground))] flex items-center">
          ...
          Integrasi Data Realisasi Master
        </h2>
        <p class="text-xs text-[color:hsl(var(--maz-muted))] mt-1">Menggabungkan seluruh transaksi E-Purchasing, Tender, Non-Tender, dan Swakelola beserta RUP-nya menjadi satu sumber kebenaran (Single Source of Truth).</p>
      </div>
      <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        ...
        <MazBtn @click="runMerge" :loading="mergeLoading" color="primary" size="sm" :disabled="!mergeStatus.allRequiredFound">
          ...
          Jalankan Merge Realisasi Master
        </MazBtn>
      </div>
    </div>
...
const fetchMergeStatus = async () => {
  try {
    ...
    const res = await $fetch('/api/admin/merge-realisasi-status', { params: { tahun } });
    if (res.success) {
      mergeStatus.value = res.data;
    }
  } catch (error) {
    console.error('Failed to load realisasi merge status', error);
  }
};
...
const runMerge = async () => {
  ...
  try {
    const tahun = selectedYear.value;
    const res = await $fetch('/api/admin/merge-realisasi-master', {
      method: 'POST',
      body: { tahun }
    });
